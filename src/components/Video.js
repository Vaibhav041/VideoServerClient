...
const Video = () => {
    ...
    useEffect(() => {
      const getVideoData = async () => {
        let videoData = await fetch(`https://videostream-y3uo.onrender.com/video/get/${id}`);
        videoData = await videoData.json();
        let channleData = await fetch(`https://videostream-y3uo.onrender.com/user/getUser/${videoData.userId}`);
